...
const [selectedLocationPickUp, setSelectedLocationPickUp] = useState<string>('New York'); // Default to New York
const [selectedLocationDropOff, setSelectedLocationDropOff] = useState<string>('New York'); // Default to New York
...
<div className="max-w-md w-full mx-auto mt-8 bg-gray-100 p-8 rounded-lg">
  <h2 className="text-3xl font-extrabold text-gray-900 text-center">Trip Duration Prediction</h2>
  <p className="mt-2 text-sm text-gray-600 text-center">Predict the duration for a trip</p>
  ...
  { predictedFare.valueOf() <= 0 ? (
        ...
        <div className="mt-8 text-center">
          <h2 className="text-2xl font-extrabold text-gray-900">Predicted duration</h2>
          <p className="mt-2 text-lg text-gray-600">
            The predicted duration is
            <span className="font-bold"> {predictedFare.toString()} minutes</span>.
          </p>
        </div>
      )}
</div>
